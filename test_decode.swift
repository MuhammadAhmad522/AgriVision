...
struct RecommendationEvidence: Codable { let url: String?; let approved: Bool?; let metric: String?; let value: Double? }
...
do {
    let result = try decoder.decode(DashboardSnapshot.self, from: json.data(using: .utf8)!)
    print("SUCCESS")
} catch {
    print("ERROR:", error)
}
