            ...
            Section(header: Text("Live Sensor Data")) {
                ...
                if viewModel.isLoading {
                    ...
                    ForEach(viewModel.readings) { reading in
                        ...
                        // \`HStack\` places items horizontally (side-by-side).
                        HStack {
                            
                            // \`VStack\` places items vertically (top-to-bottom).
                            VStack(alignment: .leading) {
                                Text(reading.type)
                                    .font(.headline) // Make it bold and readable
                                
                                Text(reading.timestamp, style: .time)
                                    .font(.caption) // Make it smaller
                                    .foregroundColor(.secondary) // Make it grey
                            }
